...
const Devolver = ({ livros, usuario, dias }: IdDevolver ): boolean => {
    const usuarioExiste = usuarios.filter(user => user.id === usuario.id)
    if (!(usuarioExiste.length > 0)) return false
    if (livros.length > 3) return false 
    ...
    if (dias > 7) {
    const atraso = dias - 7
    const multa = 5 + atraso
    console.log("❌ [FALHOU] Multa a pagar: R$", multa)
    }else {
        console.log("✔ [PASSOU] - Devolução realizada com sucesso!")
    }
...
validar({
    ...
    atual: Devolver({
        usuario: { id: 1, nome: 'Daniel' },
        livros: [ { id: 3, titulo: 'O Senhor dos Anéis - As Duas Torres' } ],
        dias: 7
